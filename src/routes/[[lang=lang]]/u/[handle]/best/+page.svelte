...
<svelte:head>
  <title>{data.handle} · Best — Movie Log</title>
</svelte:head>

{#if best.length === 0}
  <p class="text-center text-sm font-serif-italic">{t(locale, 'users.empty.best')}</p>
{:else}
  <ul class="grid gap-8">
    {#each best as movie}
      <li class="flex items-start gap-4">
        <a href={localePath(locale, `/movies/${movie.slug}`)} class="flex-1 flex items-start gap-4 no-underline group">
          {#if movie.poster}
            <img
              src={movie.poster}
              alt=""
